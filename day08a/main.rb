...
def run(input)
  ...
  map.keys.each do |antenna|
    ...
    map[antenna].combination(2).each do |loc_a, loc_b|
      ...
      if antinode_a[0] >= 0 && antinode_a[0] < rows && antinode_a[1] >= 0 && antinode_a[1] < cols
        (antinodes[antinode_a] ||= []) << antenna
      end

      if antinode_b[0] >= 0 && antinode_b[0] < rows && antinode_b[1] >= 0 && antinode_b[1] < cols
...
fail unless 14 == run(DATA.read).tap { pp(_1) }
